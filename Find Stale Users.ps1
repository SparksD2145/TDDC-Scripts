...
$PROGRAM = @{
    ...
    description = "Finds users who have not logged in within a specified period of time.";
    version = "1.0.0";
...
function beginProgram {
    ...
    # Config
    $maxDate = (Get-Date).AddMonths(-6);
    $includeNeverLoggedOn = $false;
    $excludeDisabled = $true;
    $excludePasswordNeverExpires = $true;

    Write-Verbose "Looking for users who have not logged in since $maxDate, using today as reference."

    $users = @();

    if(!$includeNeverLoggedOn){ 

        $users = Get-ADUser -Filter * -Properties LastLogonDate, PasswordNeverExpires | `
            where { $_.LastLogonDate -lt $maxDate -and $_.LastLogonDate -ne $null } | `
            sort LastLogonDate -Descending

    } else { 

        $users = Get-ADUser -Filter * -Properties LastLogonDate, PasswordNeverExpires | `
            where { $_.LastLogonDate -lt $maxDate } | `
            sort LastLogonDate -Descending

    }

    if($excludeDisabled) {
        Write-Verbose "Only including Enabled users."
        $users = $users | ? { $_.Enabled -eq $true };
    }

    if($excludePasswordNeverExpires) {
        Write-Verbose "Only users whose passwords expire."
        $users = $users | ? { $_.PasswordNeverExpires -eq $false };
    }
